...
const statusBadgeClass = (status: WorkItemStatus) => {
  switch (status) {
    case "Approved":
      return "bg-[#16A34A]";
    case "Rejected":
...
const selectClass = `${fieldInputClass} appearance-none pr-9`;
...
const WorkItems = () => {
  ...
  const handleAddNext = () => {
    navigate(AppRoutes.ADD_WORK_ITEM_DETAILS, {
      state: { workItemType: "Application" },
    });
  };
  ...
  const performSearch = () => {
    setPage(1);
    setResults(searchWorkItems(criteria));
  };
  ...
  const handleExport = () => {
    exportWorkItemsToCsv(
      results && results.length > 0 ? results : getWorkItems()
    );
  };
        ...
        <div className="flex flex-col gap-6 px-10 py-6">
          ...
          <section className="rounded-md border border-[#E5E7EB] bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 grid grid-cols-1 gap-x-8 gap-y-5 md:grid-cols-3">
              <div>
                <label className={fieldLabelClass}>Work Item ID</label>
                <input
                  type="text"
                  className={fieldInputClass}
                  value={criteria.workItemId}
                  onChange={(e) => updateField("workItemId", e.target.value)}
                />
              </div>

              <div>
                <label className={fieldLabelClass}>Work Item Type</label>
                <div className="flex h-10 w-full items-center rounded-md border border-[#D1D5DB] bg-slate-50 px-3 text-sm text-slate-800">
                  Application
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) =>
                      updateField("status", e.target.value as Criteria["status"])
                    }
                  >
              ...
              <div>
                ...
                <input
                  type="text"
                  className={fieldInputClass}
                  value={criteria.investorAccountId}
                  onChange={(e) =>
                    updateField("investorAccountId", e.target.value)
                  }
                />
              </div>

              <div>
                <label className={fieldLabelClass}>Received Date From</label>
                <div className="flex items-center gap-2">
                  <input
                    type="date"
                    className={fieldInputClass}
                    value={criteria.receivedFrom}
                  ...
                  <input
                    type="date"
                    className={fieldInputClass}
                    value={criteria.receivedTo}
                    onChange={(e) => updateField("receivedTo", e.target.value)}
                  />
                </div>
              </div>
            ...
            {expanded && (
              <div className="mt-5 grid grid-cols-1 gap-x-8 gap-y-5 md:grid-cols-3">
                <div>
                  <label className={fieldLabelClass}>Party Type</label>
                  <div className="relative">
                    <select
                      className={selectClass}
                      value={criteria.partyType}
                    ...
                    pagedResults.map((item, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          {isTooltip(item.investorName) ? (
                            <Tooltip title={item.investorName} placement="bottom">
                              <span className="cursor-default">
                                {item.investorName.slice(0, 20)}...
                              </span>
                            </Tooltip>
                          ) : (
                            item.investorName
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-block rounded-full px-3 py-0.5 text-xs font-medium text-white ${statusBadgeClass(
                              item.status
                            )}`}
                          >
                            {item.status}
                        ...
                        <td className="px-4 py-3">
                          {item.note && isTooltip(item.note) ? (
                            <Tooltip title={item.note} placement="bottom">
                              <span className="cursor-default">
        ...
        <Dialog>
          <div className="w-[440px] rounded-lg bg-white p-6 shadow-2xl">
            ...
            <p className="mt-3 text-sm leading-relaxed text-slate-600">
              {
                "You haven't entered any search criteria. Retrieving all Work Items may take a while due to the large volume of records."
              }
            </p>
